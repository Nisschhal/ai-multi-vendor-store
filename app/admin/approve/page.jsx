...
export default function AdminApprove() {
  ...
  const fetchStores = async () => {
    try {
      ...
      console.error("Error fetching stores for approval:", err)
      toast.error(
        err.response?.data?.message ||
          "Something went wrong on fetching stores for approval",
        // { id: "fetch-stores-admin" },
      )
    } finally {
      setLoading(false)
    }
  ...
  const handleApprove = async ({ storeId, status }) => {
    try {
      ...
      console.error("Error approving/rejecting store:", err)
      toast.error(
        err.response?.data?.message ||
          "Something went wrong on approving/rejecting store",
      )
    }
  }
  useEffect(() => {
